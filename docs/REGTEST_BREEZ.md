...
The embedded Lightning adapter also has isolated acceptance tests:
...
`LDK_TEST_BITCOIND` and `LDK_TEST_ELECTRS` to their executable paths. The tests
start fresh services on temporary ports, fund a channel between two embedded
nodes for each chain source, and check BOLT11 claims and failures,
BOLT12 recovery by payer note, and a restart with a queued claimable payment.
Neither test starts Spark or ldk-server. The `bitcoind` variant starts only
Bitcoin Core, with no Electrs dependency, and checks RPC password-file
precedence. To run it alone:

```sh
cargo test --locked embedded_regtest_payments_and_restart_bitcoind -- --ignored
```

The default Breez acceptance suite continues to exercise the server backend.
