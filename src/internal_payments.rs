...
impl Db {
    ...
    pub async fn reserve_internal_send(&self, send: &LightningSend) -> Result<(), String> {
        self.with(|c| {
            ...
            tx.execute("UPDATE lightning_sends SET status='SETTLING' WHERE request_id=?1",[&send.request_id])?;
            tx.commit()
        }).await
    }
    pub async fn finish_internal_send(&self, send: &LightningSend) -> Result<(), String> {
        self.with(|c| {
            let tx=c.unchecked_transaction()?;
            tx.execute("UPDATE lightning_sends SET status='SUCCEEDED',last_error=NULL WHERE request_id=?1",[&send.request_id])?;
            tx.execute("UPDATE receive_payments SET status='TRANSFER_COMPLETED' WHERE hash=?1",[&send.expected_id])?;
            tx.execute("UPDATE transfers SET status='TRANSFER_COMPLETED' WHERE request_id=(SELECT request_id FROM lightning_receives WHERE hash=?1)",[&send.expected_id])?;
            tx.commit()
        }).await
    }
}
